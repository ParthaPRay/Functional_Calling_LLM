...
def get_current_time_in_city(city: str) -> str:
    """Get the current time in a specific city using its timezone."""
    try:
        now = datetime.now(ZoneInfo(city))
        return now.strftime("%Y-%m-%d %H:%M:%S")
    except Exception as e:
        return f"Error: {str(e)}. Ensure the city format is valid, e.g., 'Europe/London'."


def get_exchange_rate(base_currency: str, target_currency: str) -> float:
    """
    Retrieves the exchange rate from a base currency to a target currency using the ExchangeRate-API.
    ...
    """
    try:
        # Replace 'your_api_key' with your actual API key from https://www.exchangerate-api.com/
        api_key = 'f64ae70defff2d8b07adc619'
        ...
        if data['result'] == 'success':
            ...
            if rate:
                ...
                return f"Error: Target currency '{target_currency}' not found."
        else:
...
get_current_time_in_city_schema = {
    "name": "get_current_time_in_city",
    "description": "Get the current time in a specific city using its IANA timezone.",
    "parameters": {
        "type": "object",
        "properties": {
            "city": {
                "type": "string",
                "description": "The IANA timezone of the city, e.g., 'Europe/London'."
            }
        },
        "required": ["city"]
    }
...
get_exchange_rate_schema = {
    ...
    "description": "Retrieve the exchange rate from a base currency to a target currency.",
    "parameters": {
        "type": "object",
        "properties": {
            "base_currency": {
...
routes = [
    Route(
        ...
        utterances=[
            ...
            "Read the content of /home/pi/somefile.txt"            
        ],
        dynamic=True,
        functions=[factorial, get_coordinates, heap_sort, list_files, get_current_time_in_city, get_exchange_rate, analyze_sentiment, read_file],
        function_schemas=[factorial_schema, get_coordinates_schema, heap_sort_schema, list_files_schema, get_current_time_in_city_schema, get_exchange_rate_schema, analyze_sentiment_schema, read_file_schema]
    )
]
...
def create_multi_shot_prompt(prompt: str, route: Route) -> str:
    ...
    Example 5:
    User: What are the coordinates of Tokyo?
    Assistant: 
    Function: get_coordinates
    ...
    Example 6:
    User: Find the GPS coordinates for Sydney
    Assistant: 
    Function: get_coordinates
    ...
    Example 8:
    User: Sort the list [5, 3, 1, 7, 9] using heap sort
    Assistant: 
    Function: heap_sort
    Arguments: {"numbers": [5, 3, 1, 7, 9]}
    Result: [1, 3, 5, 7, 9]

    Example 9:
    User: Perform heap sort on the numbers [20, 10, 30, 40, 50]
    Assistant: 
    Function: heap_sort
    Arguments: {"numbers": [20, 10, 30, 40, 50]}
    ...
    Example 11:
    User: Show me the files in the /var/log directory
    Assistant: 
    Function: list_files
    Arguments: {"directory": "/var/log"}
    Result: ["syslog", "auth.log", "kern.log", "dpkg.log"]

    Example 12:
    User: What files are in the /usr/local/bin folder?
    Assistant: 
    Function: list_files
    Arguments: {"directory": "/usr/local/bin"}
    Result: ["script.sh", "my_app", "custom_command"]

    Example 13:
    User: What is the current time in Europe/London?
    Assistant:
    Function: get_current_time_in_city
    Arguments: {"city": "Europe/London"}
    Result: "2024-09-20 12:34:56"

    Example 14:
    User: What's the time now in Asia/Tokyo?
    Assistant: 
    Function: get_current_time_in_city
    Arguments: {"city": "Asia/Tokyo"}
    Result: "2024-11-17 23:45:00"

    Example 15:
    User: Provide the current time in Australia/Sydney
    Assistant: 
    Function: get_current_time_in_city
    Arguments: {"city": "Australia/Sydney"}
    Result: "2024-11-18 00:45:00"

    Example 16:
    User: Get the exchange rate between AUD and CAD
    Assistant:
    Function: get_exchange_rate
    Arguments: {"base_currency": "AUD", "target_currency": "CAD"}
    Result: 0.95

    Example 17:
    User: Get the exchange rate between CAD and CHF
    Assistant: 
    Function: get_exchange_rate
    Arguments: {"base_currency": "CAD", "target_currency": "CHF"}
    Result: 0.72

    Example 18:
    User: What is the exchange rate from AUD to NZD?
    Assistant: 
    Function: get_exchange_rate
    Arguments: {"base_currency": "AUD", "target_currency": "NZD"}
    Result: 1.07
    
    Example 19:
    User: Analyze the sentiment of: I love this flower!
    Assistant:
    Function: analyze_sentiment
    Arguments: {"text": "I love this flower!"}
    Result: "positive" 

    Example 20:
    User: Determine the sentiment of: This is the worst experience ever
    Assistant: 
    Function: analyze_sentiment
    Arguments: {"text": "This is the worst experience ever."}
    Result: "negative"

    Example 21:
    User: What is the sentiment of: I am extremely happy with the service
    Assistant: 
    Function: analyze_sentiment
    Arguments: {"text": "I am extremely happy with the service."}
    Result: "positive"

    Example 22:
    User: Read the content of /home/pi/somefile.txt
    Assistant:
    Function: read_file
    Arguments: {"file_path": "/home/pi/somefile.txt"}
    Result: "This is the content of the file."    

    Example 23:
    User: Read the file located at /usr/share/doc/readme.txt
    Assistant: 
    Function: read_file
    Arguments: {"file_path": "/usr/share/doc/readme.txt"}
    Result: "This is the README file for documentation."

    Example 24:
    User: Display the contents of /var/log/syslog
    Assistant: 
    Function: read_file
    Arguments: {"file_path": "/var/log/syslog"}
    Result: "[System log entries...]"    
    """
    else:
